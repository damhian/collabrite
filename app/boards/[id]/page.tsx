                    ...
                    <SelectContent>
                      {["low", "medium", "high"].map((priority, key) => (
                        <SelectItem key={key} value={priority}>
                          {priority.charAt(0).toLocaleUpperCase() +
                            priority.slice(1)}
                        </SelectItem>
                      ))}
                    </SelectContent>
...
export default function BoardPage() {
  ...
  async function createTask(taskData: {
    ...
    if (!targetColumn) {
      throw new Error("No column available to create task");
    }
  ...
  async function handleDragEnd(event: DragEndEvent) {
    ...
    if (targetColumn) {
      ...
      // Check to see if we're dropping on another task
      const sourceColumn = columns.find((col) =>
        col.tasks.some((task) => task.id === taskId),
      );
  ...
  const filteredColumns = columns.map((column) => ({
    ...
    tasks: column.tasks.filter((task) => {
      ...
      if (filters.dueDate && task.due_date) {
        ...
        const filterDate = new Date(filters.dueDate).toString();
    ...
    <>
      <div className="max-h-screen bg-gray-50">
        <Navbar
          ...
          onFilterClicked={() => setIsFilterOpen(true)}
